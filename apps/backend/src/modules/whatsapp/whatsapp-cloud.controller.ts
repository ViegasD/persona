            ...
            try {
              ...
              if (imageType === 'face') {
                ...
                if (!prefs.detectedGender && occasion !== 'casal') {
                  detectFaceAttributes(buffer.toString('base64'), mimeType)
                    .then(async (attrs) => {
                      if (attrs.gender || attrs.smile) {
                        ...
                        if (!curPrefs.detectedGender) {
                          ...
                          log.info({ gender: attrs.gender, smile: attrs.smile, sessionId: session.id }, '[CLOUD WEBHOOK:IMAGE] Face attributes detected from selfie');
                        }
                      }
                    })
                    .catch((err) => log.warn({ err }, '[CLOUD WEBHOOK:IMAGE] Face attribute detection failed'));
                }
              }
            } catch (err) {
